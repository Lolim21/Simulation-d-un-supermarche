...
def finpaiment(ref, LQ, File, C1, C2,H,calendrier,TC1,TC2,ClientsQ,IX,IY,IZ,QfileIntervall,TC3,C3):
    client2 = [ref,'FP',H]
    if(LQ[0]==0):
        if(C1[0]==ref):
            C1[0]=0
...
def main(IX1,IY1,IZ1,pt):
    ...
    ratio11=[]
    ratio22=[]
    #[NCE[0]/40,NCP[0]/40,TFS/40,TSmoy/40,TATmoy/40,TauC1/40,TauC2/40,TAT_un_client/40]
    ...
    for j in range(40): #simulation 40 jours
        ...
        while (calendrier!=[]):
            ...
            if client[1] == 'A':
                ...
                finmagasinage(client[0], File,C1, C2, LQ,H,calendrier,ClientsQ,TC1,TC2,IX,IY,IZ,QfileIntervall,TC3,C3)
            else:
                clients = findclient(clients,client)
                finpaiment(client[0], LQ, File, C1, C2,H,calendrier,TC1,TC2,ClientsQ,IX,IY,IZ,QfileIntervall,TC3,C3)
        TFS = H
        interval = []
        for k in range(TFS):
            interval.append([k,0])
        ...
        MoyJ=[MoyJ[0]+NCE[0]/40,MoyJ[1]+NCP[0]/40,MoyJ[2]+TFS/40,MoyJ[3]+TSmoy/40,MoyJ[4]+TATmoy/40,MoyJ[5]+TauC1/40,MoyJ[6]+TauC2/40,MoyJ[7]+TauC3/40,MoyJ[8]+ratio1/40,MoyJ[9]+ratio2/40]
        K.append([j+1,NCE[0],NCP[0],TFS,TSmoy,TATmoy,TauC1,TauC2,TauC3,ratio1,ratio2])
    NCPbar = MoyJ[1]
    print("la moyenne des NCP :","%.4f" % NCPbar)
